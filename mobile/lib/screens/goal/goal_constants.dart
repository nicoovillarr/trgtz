const String broadcastTypeGoalUpdate = 'GOAL_UPDATED';
const String broadcastTypeGoalDelete = 'GOAL_DELETED';

const String broadcastTypeGoalCreateMilestone = 'GOAL_CREATE_MILESTONE';
const String broadcastTypeGoalDeleteMilestone = 'GOAL_DELETE_MILESTONE';
const String broadcastTypeGoalUpdateMilestone = 'GOAL_UPDATE_MILESTONE';
const String broadcastTypeGoalSetMilestones = 'GOAL_SET_MILESTONES';

const String broadcastTypeGoalReacted = 'GOAL_REACTED';
const String broadcastTypeGoalReactDeleted = 'GOAL_REACT_DELETED';

const String broadcastTypeGoalCommentCreated = 'GOAL_COMMENT_CREATED';
const String broadcastTypeGoalCommentUpdated = 'GOAL_COMMENT_UPDATED';
const String broadcastTypeGoalCommentDeleted = 'GOAL_COMMENT_DELETED';

const String broadcastTypeGoalEventAdded = 'GOAL_EVENT_ADDED';

const int MAX_COMMENT_LENGTH = 200;
